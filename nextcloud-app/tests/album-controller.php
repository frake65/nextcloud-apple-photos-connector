...
function albumControllerScenarios(PDO $pdo): void {
 ...
 $req=new class($source) implements \OCP\IRequest { public array $params; function __construct($s){$this->params=['source'=>['sourceId'=>$s,'name'=>'S'],'albums'=>[['localIdentifier'=>'album1','name'=>'Album','kind'=>'album','assets'=>['asset1','cloud:cloud-value']]]];} function getHeader(string $n):string{return $n==='Authorization'?'Basic test':'application/json';} function getParam(string $n):mixed{return $this->params[$n]??null;} function getParams():array{return $this->params;} };
 $session=new class implements \OCP\IUserSession { function getUser():mixed{return new class{function getUID():string{return 'u1';}};} };
 $c=new AlbumController($req,$session,$db); $r=$c->inventory(); if($r->getStatus()!==200) { var_export($r->getData()); echo "\n"; } check($r->getStatus()===200,'album controller accepts valid source/asset'); $r2=$c->inventory(); check($r2->getStatus()===200 && (int)$pdo->query('SELECT COUNT(*) FROM apc_source_albums')->fetchColumn()===1 && (int)$pdo->query('SELECT COUNT(*) FROM apc_album_memberships')->fetchColumn()===2,'album controller is idempotent');
